...
<x-dynamic-component :component="$getFieldWrapperView()" :field="$field">
    <div wire:ignore ax-load x-ignore
        ax-load-css="{{ \Filament\Support\Facades\FilamentAsset::getStyleHref('pill-select-styles', 'solution-forest/filament-pill-select') }}"
        class="filament-pill-select" x-data="{
        ...
        }" x-init="updateVisibleCheckboxListOptions()
        ...
        @if (!$isDisabled)
            @if ($isSearchable)
                <x-filament::input.wrapper inline-prefix prefix-icon="heroicon-m-magnifying-glass"
                    prefix-icon-alias="forms:components.checkbox-list.search-field" class="mb-4">
                    <x-filament::input inline-prefix :placeholder="$getSearchPrompt()" type="search" :attributes="\Filament\Support\prepare_inherited_attributes(
        ...
        <div class="flex gap-x-3 gap-y-2 w-full flex-wrap cursor-pointer">
            ...
            @forelse ($getOptions() as $value => $label)
                <div wire:key="{{ $this->getId() }}.{{ $statePath }}.{{ $field::class }}.options.{{ $value }}"
